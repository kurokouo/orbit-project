...
`inject_into` works on the raw stitched curve, and detrending happens after,
so the filter chews on the fake signal exactly like it would a real one. get
that backwards and completeness comes out too high with nothing to show for
it: no crash, no error, just a wrong number that looks fine.
"""
...
# Sub-samples per exposure. See Kipping (2010)
...
class InjectedSignal:
    ...
    def __post_init__(self) -> None:
        if self.period <= 0 or self.rp_earth <= 0:
            raise ValueError(f"period and rp_earth must be positive, got {self}")
        if self.impact < 0:
...
def build_transit_model(
    ...
    """Normalized flux, 1.0 out of transit, sampled at `time_days`.

    `exp_time_days` drives batman's supersampling and defaults to Kepler long
    cadence. Set it to the exposure time of whatever mission the curve came
    from.
    """
